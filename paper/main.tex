...
difficulty that survived two real optimization fixes.

A further, exhaustive follow-up ruled out every remaining optimization
explanation at the exact failing point (k=2, spacelike, $p=0.0025$): 8
different seeds all fail identically ($0.477$--$0.493$, not a seed
lottery); \texttt{lr=1e-4} with 40 epochs still fails ($0.4908$); 60 epochs
at \texttt{lr=3e-4} still fails ($0.4907$); a new sum-pooling option
(testing whether mean+max pooling discards parity-relevant information
across the $\approx 18$ largely-disconnected components a $k=2$ shot's
graph typically fragments into, versus $\approx 4$ at the working $k=1$
setting) does not help ($0.471$--$0.477$); and a much larger spatial
connectivity radius barely changes the component count (the components are
separated in time, not space). Most decisively: the training loss curve is
not stuck -- it starts at exactly $\ln 2$ and decreases steadily over the
first 15 epochs, a real if very slow trend -- but a \textbf{300-epoch run
(15$\times$ the normal budget) testing whether that trend continues to
something useful still gives $0.4914$, essentially chance}. More
optimization budget, in every form tried, does not fix this. The most
...
GNN's message passing only shares information \emph{within} a connected
component -- cross-component combination happens only at the final pooling
step, which has to implicitly learn something parity-like across many
largely-independent pieces of evidence, a well-documented hard case for
gradient-based learning generally, not specific to this codebase. The most
promising untried fix is a virtual "boundary" supernode connected to every
detector node, giving message passing itself (not just pooling) a channel
between components, mirroring MWPM's own boundary node -- a real
graph-construction change, not attempted here. Full numbers:
\texttt{results/LR\_INSTABILITY\_DIAGNOSTIC.md},
\texttt{results/K2\_TRAINING\_DEEP\_DIVE.md},
